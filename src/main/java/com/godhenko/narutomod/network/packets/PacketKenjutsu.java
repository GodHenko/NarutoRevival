...
public class PacketKenjutsu {
    private int data;

    public PacketKenjutsu() {
    }

    public PacketKenjutsu(int data) {
        this.data = data;
    }

    public static void encode(PacketKenjutsu message, PacketBuffer buf) {
        buf.writeInt(message.data);
    }

    public static PacketKenjutsu decode(PacketBuffer buffer) {
        return new PacketKenjutsu(buffer.readInt());
    }

    public static void handle(PacketKenjutsu message, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
