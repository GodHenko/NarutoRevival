...
public class PacketSetHealth {
    private int data;

    public PacketSetHealth() {
    }

    public PacketSetHealth(int data) {
        this.data = data;
    }

    public static void encode(PacketSetHealth message, PacketBuffer buf) {
        buf.writeInt(message.data);
    }

    public static PacketSetHealth decode(PacketBuffer buffer) {
        return new PacketSetHealth(buffer.readInt());
    }

    public static void handle(PacketSetHealth message, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
